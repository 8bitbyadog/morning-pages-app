...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function downloadPDF() {
        ...
        const capturedCanvas = await html2canvas(analysisEl, { scale: 2, backgroundColor: null });
        ...
        doc.text('Morning Pages', margin, margin + 7);
        ...
        const statLine = minutesSpent > 0
            ...
            : `${wordCount} words`;
        doc.text(statLine, margin, margin + 21);

        doc.setDrawColor(200, 200, 200);
        doc.line(margin, margin + 26, pageW - margin, margin + 26);

        // Analysis image — scale to fit width, cap at page height
        const imgY = margin + 31;
        ...
        doc.text(`Generated by Morning Pages App · ${new Date().toISOString()}`, margin, pageH - 6);

        doc.save(`morning-pages-${new Date().toISOString().split('T')[0]}.pdf`);
    }
