...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Download session as a self-contained HTML report
...
<body>
  ...
  <footer>Generated by Morning Pages App · ${new Date().toISOString()}</footer>
    ...
    // Download session as a Markdown file
    function downloadMarkdown() {
        ...
        let md = `# Morning Pages\n\n**${date}** · ${wordCount} words\n\n---\n\n`;
        md += `## Writing\n\n${text}\n\n---\n\n`;

        // Emotions table
        md += `## Emotional Tone\n\n`;
        if (window.emotionsChart && window.emotionsChart.data.labels.length) {
            md += `| Emotion | % |\n|---------|---|\n`;
            window.emotionsChart.data.labels.forEach((label, i) => {
                const val = window.emotionsChart.data.datasets[0].data[i];
                md += `| ${label} | ${typeof val === 'number' ? val.toFixed(1) : val} |\n`;
            });
        } else {
            md += `_No emotion data yet._\n`;
        }
        md += `\n`;

        // Topics table
        md += `## Topic Distribution\n\n`;
        if (window.topicsChart && window.topicsChart.data.labels.length) {
            md += `| Topic | % |\n|-------|---|\n`;
            window.topicsChart.data.labels.forEach((label, i) => {
                const val = window.topicsChart.data.datasets[0].data[i];
                md += `| ${label} | ${typeof val === 'number' ? val.toFixed(1) : val} |\n`;
            });
        } else {
            md += `_No topic data yet._\n`;
        }
        md += `\n`;
        ...
        if (Array.isArray(keywords) && keywords.length) {
            ...
            md += `_No keywords yet._\n`;
        }
        md += `\n`;

        // Writing speed summary
        md += `## Writing Speed\n\n`;
        if (writingSpeedData.length) {
            const speeds = writingSpeedData.map(d => d.speed).filter(s => isFinite(s) && s > 0);
            if (speeds.length) {
                const avg = speeds.reduce((a, b) => a + b, 0) / speeds.length;
                const max = Math.max(...speeds);
                const duration = writingSpeedData[writingSpeedData.length - 1].time;
                md += `- **Average pace:** ${Math.round(avg)} wpm\n`;
                md += `- **Peak pace:** ${Math.round(max)} wpm\n`;
                md += `- **Duration:** ~${Math.round(duration)} min\n`;
            }
        } else {
            md += `_No speed data yet._\n`;
        }
        md += `\n---\n\n_Generated by Morning Pages App · ${new Date().toISOString()}_\n`;
    ...
    document.getElementById('download-md-btn').addEventListener('click', downloadMarkdown);
    document.getElementById('celebration-html-btn').addEventListener('click', downloadHTML);
